...
export default async function handler(req, res) {
  ...
  if (pendErr) {
    console.error('Supabase select pending error:', pendErr);
  ...
  for (const row of pending) {
    ...
    // 2) Consultar Tiendanube
    let orderData;
    try {
      const resp = await fetch(
        `https://api.tiendanube.com/v1/${store_id}/orders/${checkout_id}` +
        '?fields=id,number,token,contact_name,contact_phone,shipping_store_branch_name,token,' +
        'shipping_pickup_type,shipping_store_branch_extra,shipping_carrier_name,' +
        'completed_at,total,products,payment_status,shipping_status',
        {
          method: 'GET',
          headers: {
    ...
    try {
      ...
      console.log('Enviando a Bubble', payload);
      ...
      if (!bubbleResp.ok) {
        console.warn(`Bubble devolvió ${bubbleResp.status}`);
        continue;
      }
